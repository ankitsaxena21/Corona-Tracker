...
export const Deaths = () => {
  ...
  return (
    <div id="Deaths">
      <div><h2 style={{ color: 'red' }}>Deaths</h2></div>
      <div><h4 style={{ color: 'red' }}>↑ {(newdatedeaths - olddatedeaths).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}</h4></div>
      <div><h1 style={{ color: 'red' }}>{newdatedeaths.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}</h1></div>
