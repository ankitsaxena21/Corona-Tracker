...
export const Recovered = () => {
  ...
  var olddaterecovered=datagather2()
    return (
        <div id="Recovered">
          <div><h2 style={{color:'green'}}>Recovered</h2></div>
          <div><h4 style={{color:'green'}}>↑ {Math.abs(olddaterecovered-newdaterecovered).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}</h4></div>
    <div><h1 style={{color:'green'}}>{newdaterecovered.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}</h1></div>
