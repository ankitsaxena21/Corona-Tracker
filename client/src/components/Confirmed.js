...
export const Confirmed = (props) => {
  ...
  var olddateconfirmed=datagather2()
    return (
        <div id="Confirmed">
          <div id="confirmed-text"><h2 style={{color:'blue'}}>Confirmed</h2></div>
          <div><h4 style={{color:'blue'}}>↑ {(newdateconfirmed-olddateconfirmed).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}</h4></div>
          <div><h1  style={{color:'blue'}}>{newdateconfirmed.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}</h1></div>
